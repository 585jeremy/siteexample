...
function routeSlug(){const slug=(window.location.hash||"").replace(/^#\/?/,"").split("/")[0].toLowerCase();return !slug?DEFAULT_ROUTE:slug===DEFAULT_ROUTE||slug===PASSWORD_ROUTE||slug===APPLICATION_ROUTE||teamsBySlug[slug]?slug:DEFAULT_ROUTE;}
...
function gateView(){
  ...
  return `<section class="staff-view staff-gate"><article class="staff-gate__card"><div class="staff-gate__layout"><div class="staff-gate__copy"><span class="gate-kicker">General staff access</span><h1 class="staff-heading">Staff portal login</h1><p class="staff-copy">Sign in to open the internal dashboard. Team workspaces are selected after login.</p></div><div class="staff-gate__panel"><form class="gate-form" data-form="staff-gate" autocomplete="off"><label class="gate-form__field"><span>Username</span><input class="gate-form__input" type="text" name="staffId" placeholder="Enter your username" /></label><label class="gate-form__field"><span>Password</span><input class="gate-form__input" type="password" name="password" placeholder="Enter your password" /></label>${state.gate.error?`<p class="gate-form__error">${esc(state.gate.error)}</p>`:""}<p class="gate-form__hint">${esc(hint)}</p><div class="staff-panel__actions"><button class="staff-panel__button staff-panel__button--primary gate-form__submit" type="submit" ${configured?"":"disabled"}>Open dashboard</button><a class="staff-panel__button" href="#/${APPLICATION_ROUTE}">Staff application</a></div></form></div></div></article></section>`;
}
...
function applicationView(){
  return `<section class="staff-view staff-gate"><article class="staff-gate__card"><div class="staff-gate__layout"><div class="staff-gate__copy"><span class="gate-kicker">Staff application</span><h1 class="staff-heading">Staff application form</h1><p class="staff-copy">Fill this out to prepare your application. Sending is blocked for now until the real backend intake is ready.</p></div><div class="staff-gate__panel"><form class="gate-form gate-form--application" autocomplete="off"><label class="gate-form__field"><span>Discord username</span><input class="gate-form__input" type="text" placeholder="Your Discord username" /></label><label class="gate-form__field"><span>In-game name</span><input class="gate-form__input" type="text" placeholder="Your in-game name" /></label><label class="gate-form__field"><span>In-game level</span><input class="gate-form__input" type="number" min="1" placeholder="Your level" /></label><label class="gate-form__field"><span>Playtime (hours)</span><input class="gate-form__input" type="number" min="0" placeholder="Your total playtime" /></label><label class="gate-form__field"><span>Main timezone</span><input class="gate-form__input" type="text" placeholder="Example: CET / EST" /></label><label class="gate-form__field"><span>Availability</span><input class="gate-form__input" type="text" placeholder="Days / hours you are usually online" /></label><label class="gate-form__field"><span>Moderation experience</span><textarea class="gate-form__input gate-form__textarea" rows="4" placeholder="Describe any moderation experience you have."></textarea></label><label class="gate-form__field"><span>Testing experience</span><textarea class="gate-form__input gate-form__textarea" rows="4" placeholder="Describe any testing / bug-reporting experience you have."></textarea></label><label class="gate-form__field"><span>Why do you want to join staff?</span><textarea class="gate-form__input gate-form__textarea" rows="5" placeholder="Tell us why you would be a good fit."></textarea></label><p class="gate-form__hint">Applications are prepared here only right now. The send button stays blocked until the live intake route is added.</p><div class="staff-panel__actions"><button class="staff-panel__button staff-panel__button--primary" type="button" disabled>Send application</button><a class="staff-panel__button" href="#/dashboard">Back</a></div></form></div></div></article></section>`;
}
...
function render(){
  ...
  if(current===APPLICATION_ROUTE){app.innerHTML=applicationView();return;}

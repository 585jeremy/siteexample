...
{slug:"admin",short:"AD",name:"Admin Team",roleId:"1479481021383835771",eyebrow:"Operations control",summary:"High-level moderation review, linked identity oversight, appeals, and prepared live moderation mirrors without Management's full command stack.",preview:["Prepared live moderation mirror","Appeal and review board","Identity integrity desk"],ch:[["Admin operations","Sensitive cases, escalations, and difficult operational decisions."],["Appeal and review","Appeals, case summaries, and final review flow."],["Identity integrity","Discord, FiveM, and account-link review for suspicious states."],["Admin escalation bridge","Escalation route into Management or Security."]],fn:[["Prepared live moderation mirror","Prepared bot-fed moderation wall for warnings, bans, and role changes.","prepared"],["Appeal decision board","Structured board for appeal triage, evidence, and resolution tracking.","ready"],["Linked account review","Review Discord-to-FiveM links and suspicious account states.","ready"],["Admin oversight log","Prepared action history for major admin decisions.","prepared"]],live:[["Case escalation queue","Active admin queue for cases passed up from Moderation or Helper.","ready"],["Bot moderation log","Prepared Discord-bot mirror for higher-level staff actions.","prepared"],["Identity exception alerts","Prepared anomaly lane for mismatched or broken account links.","prepared"],["Admin notes","Sensitive internal notes and resolution follow-ups.","ready"]]},
...
const state={gate:{loggedIn:false,account:null,error:""},discord:{status:"idle",user:null,roles:[],syncStatus:"",verifiedAt:"",error:""}};

function esc(value){return String(value??"").replace(/[&<>\"']/g,(char)=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[char]));}
function normalize(value){return String(value??"").trim().toLowerCase();}
function routeSlug(){const slug=(window.location.hash||"").replace(/^#\/?/,"").split("/")[0].toLowerCase();return !slug?DEFAULT_ROUTE:slug===DEFAULT_ROUTE||teamsBySlug[slug]?slug:DEFAULT_ROUTE;}
...
function saveStaffSession(account){const payload={username:account.username,displayName:account.displayName||account.name||account.username,clearance:account.clearance||"General Staff",issuedBy:account.issuedBy||"Management Team"};sessionStorage.setItem(GATE_KEY,JSON.stringify(payload));state.gate.loggedIn=true;state.gate.account=payload;state.gate.error="";}
...
function hasTeamAccess(team){return hasDiscordSession()&&(hasManagementAccess()||state.discord.roles.includes(team.roleId));}
...
function shellMode(){shell.classList.toggle("is-gated",!state.gate.loggedIn);}
...
function gateView(){
  const configured=GATE_ACCOUNTS.length>0;
  return `<section class="staff-view staff-gate"><article class="staff-gate__card"><div class="staff-gate__layout"><div class="staff-gate__copy"><span class="gate-kicker">General staff access</span><h1 class="staff-heading">Staff access starts here.</h1><p class="staff-copy">Use the private staff credentials sent by management. This first step only opens the internal dashboard. Entering an actual team workspace still needs Discord role verification, so nobody can open the wrong team with only the staff password.</p><div class="staff-inline-notes"><span class="staff-inline-note">No floating team bar before login</span><span class="staff-inline-note">Discord check only when entering a team</span><span class="staff-inline-note">Management can override every workspace after verification</span></div></div><div class="staff-gate__panel"><span class="gate-kicker gate-kicker--soft">Manager-issued credentials</span><form class="gate-form" data-form="staff-gate" autocomplete="off"><label class="gate-form__field"><span>Staff ID</span><input class="gate-form__input" type="text" name="staffId" placeholder="Enter your staff ID" /></label><label class="gate-form__field"><span>Password</span><input class="gate-form__input" type="password" name="secret" placeholder="Enter your staff password" /></label>${state.gate.error?`<p class="gate-form__error">${esc(state.gate.error)}</p>`:""}<p class="gate-form__hint">${configured?"This gate opens the dashboard only. Team entry stays Discord role-locked.":"No staff credentials are configured yet. Add manager-issued accounts in staff-gate-config.js."}</p><button class="staff-panel__button staff-panel__button--primary gate-form__submit" type="submit" ${configured?"":"disabled"}>Enter staff dashboard</button></form></div></div></article></section>`;
}
...
function dashboardView(){
  ...
  const discordState=hasDiscordSession()?verifiedIdentity():state.discord.status==="error"?"Discord verification currently unavailable":"No Discord team verification yet";
  const account=state.gate.account||{displayName:"Staff Access",clearance:"General Staff"};
  return `<section class="staff-view staff-dashboard"><section class="dashboard-hero"><article class="staff-panel staff-panel--hero"><span class="gate-kicker">General staff dashboard</span><h1 class="staff-heading">${esc(account.displayName)}, choose your team.</h1><p class="staff-copy">You are inside the general staff portal now. Every team is visible here, but the actual workspace only opens after the matching Discord role is verified. Management clearance unlocks the full stack once the Management Team role is confirmed.</p><div class="staff-panel__actions">${hasManagementAccess()?`<span class="staff-state-pill staff-state-pill--open">Management override active</span>`:`<span class="staff-state-pill staff-state-pill--prepared">Choose a team below</span>`}</div></article><div class="dashboard-hero__rail">${statCard("Staff gate",account.clearance||"General Staff","The first layer is open with manager-issued credentials.")}${statCard("Discord state",hasDiscordSession()?"Verified":state.discord.status==="error"?"Offline":"Pending",discordState)}${statCard("Unlocked teams",String(unlockedCount),hasManagementAccess()?"Management verification unlocks every team.":"Unlocked count changes only after Discord role checks.")}</div></section><section class="team-grid">${teams.map((team)=>teamCard(team)).join("")}</section></section>`;
}
...
function workspaceView(team){
  const override=hasManagementAccess()&&!state.discord.roles.includes(team.roleId);
  const statusItems=[
    ["General staff gate",state.gate.account?.displayName||"Staff access active","ready"],
    ["Verified Discord",verifiedIdentity(),"ready"],
    ["Role state",override?"Management override active":`${team.name} verified`,"ready"],
    ["Last verification",state.discord.verifiedAt?new Date(state.discord.verifiedAt).toLocaleString("en-GB"):"Verified this session","ready"],
  ];
  return `<section class="staff-view workspace-view"><section class="workspace-hero"><article class="staff-panel staff-panel--hero"><span class="gate-kicker">${esc(team.eyebrow)}</span><h1 class="staff-heading">${esc(team.name)}</h1><p class="staff-copy">${esc(team.summary)}${override?" Management verification is overriding the direct team role for this workspace.":""}</p><div class="staff-panel__actions"><span class="staff-state-pill staff-state-pill--open">${override?"Management override":"Role verified"}</span><a class="staff-panel__button" href="#/dashboard">Back to dashboard</a></div></article><div class="dashboard-hero__rail">${statCard("Workspace state","Open",override?"Management clearance is unlocking this team.":"This team role is verified for the current Discord account.")}${statCard("Discord identity",verifiedIdentity(),"Team access is tied to the live Discord session, not only the general staff gate.")}${statCard("Live readiness",team.live.some((item)=>item[2]==="prepared")?"Prepared":"Ready","Prepared surfaces are already scaffolded for future server and bot data.")}</div></section><section class="workspace-columns">${workspaceSection("Channels",`${team.name} channels`,"Internal channels and working lanes for this role.",team.ch.map(([title,body])=>[title,body,"ready"]))}${workspaceSection("Functions",`${team.name} control surfaces`,"Role-specific tools, boards, and operational surfaces.",team.fn)}${workspaceSection("Prepared live surfaces",`${team.name} live`,"Prepared live data, bot-driven mirrors, and future operational feeds for this role.",team.live)}${workspaceSection("Access frame","Verification state","General staff credentials open the portal. Discord role verification opens the actual team workspace.",statusItems)}</section></section>`;
}
...
function handleStaffGateSubmit(form){
  const formData=new FormData(form);
  const staffId=normalize(formData.get("staffId"));
  const secret=String(formData.get("secret")||"");
  const matched=GATE_ACCOUNTS.find((account)=>normalize(account.username)===staffId&&String(account.password||"")===secret);
  if(!matched){
    state.gate.error="That staff ID and password pair does not match the manager-issued access list.";
    render();
    return;
  }
  saveStaffSession(matched);
...
document.addEventListener("click",(event)=>{
  ...
  if(action.dataset.action==="logout-staff"){
    clearStaffSession();
    state.gate={loggedIn:false,account:null,error:""};
    state.discord={status:"idle",user:null,roles:[],syncStatus:"",verifiedAt:"",error:""};
    window.location.hash="#/dashboard";
    render();
  }
...
loadStaffSession();
render();
if(state.gate.loggedIn)loadDiscordState();
